...
// Convert decimal feet to { ft, inch }
function toFtIn(px) {
...
function FtInRow({ label, px, onChange }) {
  ...
  const apply = (newFt, newIn) => {
    ...
    if (v >= PX_PER_FOOT * 0.25) onChange(v); // min 0.25 ft
  };
  return (
    <Row label={label}>
...
export default function PropertiesPanel({ item, onUpdate, onDelete, onDuplicate }) {
  ...
  const snapRotate = (deg) => {
    onUpdate({ rotation: (Math.round(rotation / deg) * deg + 360) % 360 });
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: '12px 12px' }}>
        ...
        {/* Color — show for non-room types or rooms */}
        <Row label="Color">
