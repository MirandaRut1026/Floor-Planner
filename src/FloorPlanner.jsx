...
import {
  ...
  Menu, Grid3x3, DoorOpen, Plus, Sparkles, Building2, ZoomIn, ZoomOut
} from 'lucide-react';
...
// ───────── Pre-loaded house from your dad's specs ─────────
// Numbers like 14.1 read as 14 ft 1 in. Extra wall lengths suggest
// closet jogs / alcoves you'll need to add as small rectangles.
...
// Pre-loaded windows from your specs (Living Room back wall)
const BENBROOK_OPENINGS = [
  // 9'7" wide window on 17'4" back wall — centered for now
  { kind: 'window', name: 'Window', wIn: ftIn(9, 7), hIn: 4, x: 80 + (ftIn(17,4)*BENBROOK_PPI)/2, y: 70 },
  // 3'x3' window on side wall, 20" from back wall
...
export default function FloorPlanner() {
  ...
  // Unified items: rooms, furniture, doors, windows
  ...
  const loadBenbrookHouse = () => {
    ...
    const rooms = BENBROOK_HOUSE.map((r) => ({
      ...
      color: 'transparent',
      ...r,
      // Center coordinates instead of top-left
  ...
  const onCanvasPointerDown = (e) => {
    if (!calibrationMode) return;
    if (e.target !== e.currentTarget) return; // ignore if clicking item
    const rect = canvasRef.current.getBoundingClientRect();
    const p = { x: e.clientX - rect.left, y: e.clientY - rect.top };

    // Hit-test calibration dots
    const HIT = 22;
  ...
  const onPointerMove = (e) => {
    ...
    if (dragState.current && dragState.current.pointerId === e.pointerId) {
      const rect = canvasRef.current.getBoundingClientRect();
      const nx = e.clientX - rect.left - dragState.current.offsetX;
      const ny = e.clientY - rect.top - dragState.current.offsetY;
  ...
  const onCanvasClick = (e) => {
    if (!calibrationMode && !dragState.current && e.target === e.currentTarget) {
      setSelectedId(null);
    }
  };
  ...
  const duplicateSelected = () => {
    ...
    if (!item) return;
    const newItem = { ...item, id: newId(), x: item.x + 24, y: item.y + 24 };
  ...
  const saveLayout = async () => {
    ...
    const key = `plan:${name}`;
    const payload = { items, pixelsPerInch, name, savedAt: Date.now() };
    try {
  ...
  const renderItem = (item) => {
      ...
      return (
        ...
        >
          <span
            ...
            style={{
              fontSize: Math.min(28, Math.max(11, Math.min(w, h) / 9)),
              fontStyle: 'italic',
              fontWeight: 400,
              color: INK,
              opacity: 0.7,
          ...
          {Math.min(w, h) > 70 && (
            <span
              className="font-mono"
              style={{
                fontSize: Math.min(10, Math.max(8, Math.min(w, h) / 22)),
                color: MUTED,
                pointerEvents: 'none',
              }}
            >
    ...
    if (item.kind === 'door') {
      // Square footprint covering door + swing area. Wall is bottom edge,
      // hinge bottom-right, door panel along right edge (closed), arc shows swing.
      return (
        <div
          key={item.id}
          onPointerDown={(e) => onItemPointerDown(e, item.id)}
        ...
        >
          <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" style={{ display: 'block' }}>
            ...
            {/* Door panel (right edge, closed position) */}
            <line x1={w - 1.5} y1={h - 1.5} x2={w - 1.5} y2="0" stroke={INK} strokeWidth="2" />
            {/* Swing arc (hinge bottom-right) */}
            <path
              d={`M ${w - 1.5},0 A ${w - 1.5},${h - 1.5} 0 0 1 0,${h - 1.5}`}
              fill="none"
              stroke={ACCENT}
              strokeWidth="1.2"
              strokeDasharray="3 3"
      ...
      return (
        <div
          ...
          style={{
            ...baseStyle,
            background: '#E8F2F8',
            border: isSelected ? `2px solid ${ACCENT}` : `1.5px solid ${INK}`,
            zIndex: 4,
          }}
        >
      ...
      <div
        ...
        style={{
          ...
          boxShadow: isSelected
            ? `0 0 0 4px ${ACCENT}33, 0 2px 8px ${INK}22`
            : `0 1px 2px ${INK}15`,
          zIndex: 5,
        }}
      >
        <span
          className="font-mono"
          style={{
            fontSize: Math.min(10, Math.max(7, Math.min(w, h) / 6)),
  ...
  // Sorted: rooms (bg), then openings, then furniture (top)
  ...
  const gridSize = pixelsPerInch * 12; // 1 ft
  ...
  return (
    ...
    >
      ...
      <header
        ...
        style={{ padding: '10px 14px', background: PAPER, borderBottom: `1px solid ${LINE}`, minHeight: 56 }}
      >
        <div className="flex items-center gap-2 min-w-0">
          <button
            onClick={() => setSidebarOpen((v) => !v)}
            style={iconBtn(sidebarOpen && !isWide ? INK : 'transparent', sidebarOpen && !isWide ? PAPER : INK, sidebarOpen && !isWide ? INK : LINE)}
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <input
              ...
              className="font-display"
              style={{ background: 'transparent', border: 'none', fontSize: 18, fontWeight: 500, color: INK, padding: '4px 0', minWidth: 0, width: '100%', maxWidth: 240 }}
            />
          </div>
        </div>
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <button onClick={() => setPixelsPerInch((p) => Math.max(0.5, p * 0.85))} style={iconBtn('transparent', INK, LINE)} aria-label="Zoom out"><ZoomOut size={16} /></button>
          <span className="font-mono" style={{ fontSize: 10, color: MUTED, width: 36, textAlign: 'center' }}>
            {Math.round((pixelsPerInch / DEFAULT_PPI) * 100)}%
          </span>
          <button onClick={() => setPixelsPerInch((p) => Math.min(8, p * 1.18))} style={iconBtn('transparent', INK, LINE)} aria-label="Zoom in"><ZoomIn size={16} /></button>
          <div style={{ width: 1, height: 24, background: LINE, margin: '0 4px' }} />
          <button onClick={() => setShowGrid(!showGrid)} style={iconBtn(showGrid ? INK : 'transparent', showGrid ? PAPER : INK, showGrid ? INK : LINE)} aria-label="Toggle grid"><Grid3x3 size={16} /></button>
      ...
      <div className="flex-1 flex relative overflow-hidden">
        ...
        {sidebarOpen && (
          <aside
            ...
            style={{
              width: 320, maxWidth: '85vw', background: PAPER, borderRight: `1px solid ${LINE}`,
              overflowY: 'auto',
              position: isWide ? 'relative' : 'absolute',
              top: 0, bottom: 0, left: 0, zIndex: 40,
              boxShadow: !isWide ? `0 0 40px ${INK}33` : 'none',
              touchAction: 'pan-y',
            }}
          >
            ...
            {/* PLAN section (image + calibration) */}
            <Section
              title="PLAN IMAGE"
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                <button onClick={() => fileInputRef.current?.click()} style={solidBtn()}>
                  <Upload size={15} />
                  {backgroundImage ? 'Replace image' : 'Upload plan image'}
                </button>
                {backgroundImage && (
                  <>
                    <button onClick={() => calibrationMode ? cancelCalibration() : setCalibrationMode(true)}
                      style={{ ...outlineBtn(calibrationMode ? ACCENT : LINE), background: calibrationMode ? ACCENT : 'transparent', color: calibrationMode ? PAPER : INK }}>
                      <Ruler size={15} />
                      {calibrationMode ? 'Cancel scaling' : 'Set scale'}
                    </button>
            ...
            <Section
              ...
              onToggle={() => setExpandedSections((p) => ({ ...p, rooms: !p.rooms }))}
            >
              <div className="flex flex-col gap-2">
            ...
            <Section
              title="DOORS &amp; WINDOWS"
              expanded={expandedSections.openings}
              onToggle={() => setExpandedSections((p) => ({ ...p, openings: !p.openings }))}
            >
              <div className="flex flex-col gap-2">
                <button
                  onClick={() => addItem({ kind: 'door', name: 'Door', wIn: 36, hIn: 36, color: '#8B4513' })}
                  style={outlineBtn(LINE)}
                >
                  <DoorOpen size={15} /> Add door (36")
                </button>
                <button
                  onClick={() => addItem({ kind: 'door', name: 'Door', wIn: 30, hIn: 30, color: '#8B4513' })}
                  style={outlineBtn(LINE)}
                >
                  <DoorOpen size={15} /> Add door (30")
                </button>
                <button
                  onClick={() => addItem({ kind: 'window', name: 'Window', wIn: 36, hIn: 6, color: '#8DB3D1' })}
                  style={outlineBtn(LINE)}
                >
                  <WindowIcon /> Add window (36")
                </button>
                <button
                  onClick={() => addItem({ kind: 'window', name: 'Window', wIn: 60, hIn: 6, color: '#8DB3D1' })}
                  style={outlineBtn(LINE)}
                >
              ...
              <p className="font-mono" style={{ fontSize: 10, color: MUTED, marginTop: 10, lineHeight: 1.5 }}>
                EDIT THE SIZE EXACTLY ONCE PLACED. ROTATE 90° TO PUT ON A SIDE WALL.
              </p>
            </Section>
            ...
            <Section
              title="FURNITURE"
              expanded={expandedSections.furniture}
              onToggle={() => setExpandedSections((p) => ({ ...p, furniture: !p.furniture }))}
            >
              <button
                onClick={() => setCustomItemOpen(true)}
                style={{ ...outlineBtn(INK), borderStyle: 'dashed', marginBottom: 8 }}
              >
                  ...
                  return (
                    <div key={cat}>
                      <button
                        ...
                        style={{ minHeight: 40, padding: '0 4px', background: 'transparent', border: 'none', cursor: 'pointer', color: INK, textAlign: 'left' }}
                      >
                      ...
                      {open && (
                        <div className="flex flex-col gap-1 pl-3 pb-2">
                          {catItems.map((it) => (
                            <button
                              key={it.name}
                              onClick={() => addItem({ kind: 'furniture', name: it.name, wIn: it.w, hIn: it.h, color: it.color })}
                              className="item-tap flex items-center justify-between text-left"
                              style={{ minHeight: 44, padding: '0 12px', background: 'transparent', border: `1px solid ${LINE}`, fontSize: 13, cursor: 'pointer', color: INK }}
                            >
        ...
        <main className="flex-1 flex flex-col relative" style={{ background: CANVAS_BG, minWidth: 0 }}>
          {calibrationMode && (
            <div className="px-4 py-2 flex items-center justify-center gap-3 anim-fade flex-wrap" style={{ background: ACCENT, color: PAPER }}>
              ...
              <button
                ...
                className="font-mono"
                style={{
                  background: snapStraight ? PAPER : 'transparent',
                  color: snapStraight ? ACCENT : PAPER,
          ...
          <div
            ...
            style={{
              ...
              backgroundImage: showGrid ? `linear-gradient(${LINE}66 1px, transparent 1px), linear-gradient(90deg, ${LINE}66 1px, transparent 1px)` : 'none',
          ...
          >
            {backgroundImage && (
              <img
                ...
                style={{
                  ...
                  opacity: bgOpacity, pointerEvents: 'none', userSelect: 'none',
                }}
              />
            )}
            ...
            {!backgroundImage && items.length === 0 && (
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none px-8" style={{ color: MUTED, textAlign: 'center' }}>
                <div className="font-display mb-2" style={{ fontSize: 38, fontStyle: 'italic', fontWeight: 300, color: INK, opacity: 0.2 }}>
                  begin here
                </div>
            ...
            {calibrationMode && (
              <>
                ...
                {calibrationPoints.length === 2 && (
                  <div
                    className="absolute flex items-center gap-2 px-3 py-2 anim-fade"
                    style={{ top: 16, left: '50%', transform: 'translateX(-50%)', background: INK, color: PAPER, boxShadow: `0 4px 16px ${INK}66` }}
                  >
                    ...
                    <input
                      ...
                      onKeyDown={(e) => { if (e.key === 'Enter') completeCalibration(); }}
                      className="font-mono"
                      style={{ background: 'transparent', border: 'none', borderBottom: `1px solid ${PAPER}`, color: PAPER, fontSize: 16, width: 60, textAlign: 'right', padding: '4px 6px' }}
                    />
            ...
            <div className="absolute flex items-center" style={{ bottom: 14, left: 14, pointerEvents: 'none' }}>
              <div style={{ width: pixelsPerInch * 12 * 5, height: 4, background: INK }} />
              <span className="font-mono" style={{ fontSize: 10, color: INK, marginLeft: 8 }}>5 ft</span>
            </div>

            {/* Item count */}
            <div className="absolute font-mono" style={{ top: 14, right: 14, fontSize: 10, color: MUTED, letterSpacing: '0.1em', background: `${PAPER}cc`, padding: '4px 8px', pointerEvents: 'none' }}>
          ...
          {selected && (
            ...
            >
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="font-display"
                    style={{ background: 'transparent', border: 'none', borderBottom: `1px solid ${LINE}`, fontSize: 19, fontWeight: 500, color: INK, padding: '2px 0', maxWidth: 220 }}
                  />
                </div>
              ...
              {/* Always-visible size editor */}
              <div className="flex items-center gap-3 mb-3 flex-wrap">
                <DimEditor
                  label="W"
                  totalInches={selected.wIn}
                  onChange={(v) => updateSelected({ wIn: Math.max(1, v) })}
                />
                <DimEditor
                  label="H"
                  totalInches={selected.hIn}
                  onChange={(v) => updateSelected({ hIn: Math.max(1, v) })}
                />
                {selected.kind !== 'room' && selected.kind !== 'window' && selected.kind !== 'door' && (
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="color" value={selected.color || '#888'}
                      onChange={(e) => updateSelected({ color: e.target.value })}
                      style={{ width: 36, height: 32, padding: 0, border: `1px solid ${LINE}`, background: 'transparent', cursor: 'pointer' }}
                    />
                  </div>
                )}
              </div>
  ...
  return (
    <div style={{ borderBottom: `1px solid ${LINE}` }}>
      <button
        ...
        style={{ padding: '14px 20px', background: 'transparent', border: 'none', cursor: 'pointer', color: INK }}
      >
        <span className="font-mono" style={{ fontSize: 10, color: MUTED, letterSpacing: '0.12em', fontWeight: 500 }}>
          {title}
        </span>
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <input
        ...
        className="font-display"
        style={{ width: '100%', minHeight: 44, padding: '8px 12px', fontSize: 17, background: 'transparent', border: `1px solid ${LINE}`, color: INK, marginBottom: 16 }}
      />
...
function CustomItemModal({ onClose, onAdd }) {
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <input
        autoFocus value={name} onChange={(e) => setName(e.target.value)}
        placeholder="e.g. Mom's antique cabinet"
        className="font-display"
        style={{ width: '100%', minHeight: 44, padding: '8px 12px', fontSize: 17, background: 'transparent', border: `1px solid ${LINE}`, color: INK, marginBottom: 16 }}
      />
...
function DimEditor({ label, totalInches, onChange }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <input
        type="number" inputMode="decimal" value={ft}
        onChange={(e) => {
          const v = parseFloat(e.target.value) || 0;
          onChange(v * 12 + inches);
        }}
        className="font-mono"
        style={{ width: 50, minHeight: 36, background: 'transparent', border: `1px solid ${LINE}`, padding: '4px 6px', fontSize: 14, color: INK, textAlign: 'center' }}
      />
      <span className="font-mono" style={{ fontSize: 10, color: MUTED }}>ft</span>
      <input
        type="number" inputMode="decimal" value={inches}
        onChange={(e) => {
          const v = parseFloat(e.target.value) || 0;
          onChange(ft * 12 + v);
        }}
        className="font-mono"
        style={{ width: 56, minHeight: 36, background: 'transparent', border: `1px solid ${LINE}`, padding: '4px 6px', fontSize: 14, color: INK, textAlign: 'center' }}
      />
...
// Tiny lucide-style window icon (not in default lucide set)
function WindowIcon() {
  return (
